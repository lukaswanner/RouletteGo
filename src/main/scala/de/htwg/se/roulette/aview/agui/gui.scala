package de.htwg.se.roulette.aview.agui

import de.htwg.se.roulette.controller.{CellChanged, ControllerInterface, GameStart}
import de.htwg.se.roulette.model.playboardComponent.playboardBaseImpl.Solver
...
class gui(controller: ControllerInterface) extends Frame {
  ...
  //redraw(de.htwg.se.roulette.controller.getPlayBoard().getactivePlayer())
  centerOnScreen()
  repaint()
    ...
    menuBar = new MenuBar {
      ...
      contents += new Menu("Options") {
        ...
        contents += new MenuItem(Action("read") {
          val file = controller.createFileIO()
          file.load
        })
        contents += new MenuItem(Action("save") {
          val file = controller.createFileIO()
          file.save(controller.getPlayBoard())
        })
